%% Stage 04A | Within-subject time-frequency analysis (CWT)
...
%% Stage 04A | Load preprocessed data of a single subject (035 or 003) and apply baseline correction
...
%% Stage 04A | Separate epochs by stimulus class
...
%% Stage 04A | Compute iF power for standard condition (channel-wise, epoch-wise)
...
iF_Power_epoch_standard=zeros(n_f,m,q_stand); % initialization of a variable to be
%used in the following for loops
iF_Power_mean_standard=zeros(n_f,m,n); % initialization of a variable to be used
%in the following for loops
%After computation, iF_Power_mean_standard will contains abs(cwt.^2) at each channel
...
for i=1:n %external for loop, iteration over channels
    ...
    for j=1:q_stand %internal for loop, iteration on epochs
        ...
        %the j-th epoch. xach row corresponds to a frequency value, each
        %column corresponds to a time value
        iF_Power_epoch_standard(:,:,j)=abs(C).^2; %in the j-th page, it contains
        %the iF power of the i-th channel signal in the j-th epoch
    end
    iF_Power_mean_standard(:,:,i)=mean(iF_Power_epoch_standard,3); %in the i-th page,
    %it contains the iF power of the i-th channel averaged across epochs
end

%iF_Power_mean_standard is a n_f x m x n, containing in the i-th page the
%iF power of the i-th channel, averaged across all standard epochs.
%In each page, each row refers to a frequency value,
%each column refers to a time value

%% Stage 04A | Compute iF power for target and distractor conditions
%Repeat the same workflow as in the previous section
q_targ=size(X_target,3); %number of target epochs

iF_Power_epoch_target=zeros(n_f,m,q_targ);
iF_Power_mean_target=zeros(n_f,m,n);
%After computation, iF_Power_mean_target will contains abs(cwt.^2) at each channel
%averaged across all q_targ epochs
for i=1:n
    i;
    for j=1:q_targ
        [C,F] = cwt(X_target(i,:,j),'amor',srate,'frequencylimits',flimits);
        iF_Power_epoch_target(:,:,j)=abs(C).^2;
    end
    iF_Power_mean_target(:,:,i)=mean(iF_Power_epoch_target,3);
end

%iF_Power_mean_target is a n_f x m x n, containing in the i-th page the
%iF power of the i-th channel, averaged across all target epochs.
%In each page, each row refers to a frequency value,
%each column refers to a time value

q_distr=size(X_distractor,3); %number of distractor epochs

iF_Power_epoch_distractor=zeros(n_f,m,q_distr);
iF_Power_mean_distractor=zeros(n_f,m,n);
%After computation, iF_Power_mean_distractor will contains the coefficient abs(cwt.^2) at each channel
%averaged across all q_distr epochs
for i=1:n
    i;
    for j=1:q_distr
        [C,F] = cwt(X_distractor(i,:,j),'amor',srate,'frequencylimits',flimits);
        iF_Power_epoch_distractor(:,:,j)=abs(C).^2;
    end
    iF_Power_mean_distractor(:,:,i)=mean(iF_Power_epoch_distractor,3);
end

%iF_Power_mean_distractor is a n_f x m x n, containing in the i-th page the
%iF power of the i-th channel, averaged across all distractor epochs.
%In each page, each row refers to a frequency value,
%each column refers to a time value

%% Stage 04A | Normalize each frequency using baseline mean

baseidx=[1 101]; %indices of the baseline period

baseline_standard=mean(iF_Power_mean_standard(:,baseidx(1):baseidx(2),:),2); %n_f x 1 x n
iF_Power_standard=iF_Power_mean_standard./baseline_standard; %n_f x m x n

baseline_target=mean(iF_Power_mean_target(:,baseidx(1):baseidx(2),:),2);     %n_f x 1 x n
iF_Power_target=iF_Power_mean_target./baseline_target;       %n_f x m x n
...
%% Stage 04A | Generate iF power colormaps (3x3) at Fz/Cz/Pz
...
ch=12; %Fz
subplot(331)
TF_colormap(times,frequencies,iF_Power_standard(:,:,ch),clim,['standard ',ch_names{ch}])
subplot(334)
TF_colormap(times,frequencies,iF_Power_target(:,:,ch),clim,['target ',ch_names{ch}])
subplot(337)
TF_colormap(times,frequencies,iF_Power_distractor(:,:,ch),clim,['distractor ',ch_names{ch}])
...
%% Stage 04A | Generate alpha-power topographical maps (3x8) across time


alphalim=dsearchn(F,[8 14]'); %alphalim contains the indices of the values in F
%closest to value 8 and to value 14.
iF_Power_standard_alpha=squeeze(mean(iF_Power_standard(alphalim(2):alphalim(1),:,:),1));
%iF_Power_standard_alpha is a m x n matrix (m time points x n channels)
iF_Power_target_alpha=squeeze(mean(iF_Power_target(alphalim(2):alphalim(1),:,:),1));
%iF_Power_target_alpha is a m x n matrix (m time points x n channels)
iF_Power_distractor_alpha=squeeze(mean(iF_Power_distractor(alphalim(2):alphalim(1),:,:),1));
...
for i=1:n_frames
    ...
    ta_alpha_standard=mean(iF_Power_standard_alpha(start:stop,:),1);
...
for i=1:n_frames
    ...
    start=frames(i)-0.05*srate;
    stop=frames(i)+0.05*srate;
    ta_alpha_target=mean(iF_Power_target_alpha(start:stop,:),1);
...
for i=1:n_frames
    ...
    start=frames(i)-0.05*srate;
    stop=frames(i)+0.05*srate;
    ta_alpha_distractor=mean(iF_Power_distractor_alpha(start:stop,:),1);
