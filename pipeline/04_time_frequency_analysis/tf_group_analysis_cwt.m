%% Stage 04B | Group-level time-frequency analysis (CWT)
...
%% Stage 04B | Load group-averaged time-frequency power
...
%% Stage 04B | Generate iF power colormaps (3x3) at Fz/Cz/Pz
...
ch=12; %Fz
subplot(331)
TF_colormap(times,frequencies,iF_Power_standard_GA(:,:,ch),clim,['standard ',ch_names{ch}])
subplot(334)
TF_colormap(times,frequencies,iF_Power_target_GA(:,:,ch),clim,['target ',ch_names{ch}])
subplot(337)
TF_colormap(times,frequencies,iF_Power_distractor_GA(:,:,ch),clim,['distractor ',ch_names{ch}])
...
%% Stage 04B | Generate alpha-power topographical maps (3x8) across time
