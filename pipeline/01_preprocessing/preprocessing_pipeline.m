...
%% Stage 01 | Load the data; transform from single to double precision

clear
close all
clc

load(['sub-035_ses-01_task-Rest_eeg.mat'])  %it contains X(=data), srate, stim_samples, stim_types
...
%% Stage 01 | Possible Resampling (here, resampling is not applied)
...
X=Xresampled; %at each stage, X is overwritten 
...
stim_samples=floor(stim_samples_or*p/q); %time samples of the stimuli recomputed considering the possibly new srate

%% Stage 01 | Linear Detrending

Xdetrended=detrend(X')'; 
X=Xdetrended; %at each stage, X is overwritten 

%% Stage 01 | Plot the signals over the entire duration 
...
%% Stage 01 | Compute and plot the power spectrum density of all signals and also plot the PSD of electrodes n.10,11,53
...
% f from 0 to 250 Hz with 0.1 Hz step, number of frequencies n_fr = 2501
% PSD_unfiltered: n_fr x n (each column is the power spectrum density of one channel)

%In the following figures, all  power spectra are plotted.  y-axis has been 
%limited between 0-100 microV^2/Hz to improve visualization. Without limiting y-axis, the power
%spectra would appear flat since at low frequencies they assume much larger values than
...
%Focus on F3 = 10
figure
plot(f(1:end), PSD_unfiltered(1:end,10),'linewidth',1);
xlim([0 80])
ylim([0 100])
xlabel('Hz')
ylabel('{\muV}^2/Hz')
title(['PSD of unfiltered ' ch_names{10}])

%Focus on F1 = 11
figure
plot(f(1:end), PSD_unfiltered(1:end,11),'linewidth',1);
xlim([0 80])
ylim([0 100])
xlabel('Hz')
ylabel('{\muV}^2/Hz')
title(['PSD of unfiltered ' ch_names{11}])

%Focus on PO3 = 53
figure
plot(f(1:end), PSD_unfiltered(1:end,53),'linewidth',1);
xlim([0 80])
ylim([0 100])
xlabel('Hz')
ylabel('{\muV}^2/Hz')
title(['PSD of unfiltered ' ch_names{53}])

%% Stage 01 | Filtering with FIR filter (Kaiser window) COMMENTED 
...
% Mags=[1 0];
% Devs=[0.05 0.01];
% [N,Wn,beta,ftype] = kaiserord(Fedges,Mags,Devs,srate);%the order of the FIR filter N = 56
...
%% Stage 01 | Filtering (low-pass, high-pass, notch) with IIR filters (elliptic) 
...
%% Stage 01 | Plot the filtered signals over the entire duration

[n, m] = size(X); % n=number of channels=59; m=number of samples 
...
%% Stage 01 | Compute the power spectrum density of all filtered signals and compare the PSD before vs after filtering (electr. 10,11,53)
...
% %figures with PSD of all signals before and after filtering
% %(y-axis limited between 0-100 microV^2/Hz)
figure
for i = 1:30
    subplot(5,6,i)
    hold on
    plot(f(1:end), PSD_unfiltered(1:end,i),'linewidth',1);
    plot(f(1:end), PSD_filtered(1:end,i),'linewidth',1);
    title(ch_names{i})
    xlim([0 80])
    ylim([0 100])
    xlabel('Hz')
...
plot(f(1:end), PSD_unfiltered(1:end,10),'linewidth',1);
hold on
plot(f(1:end), PSD_filtered(1:end,10),'linewidth',1);
xlim([0 80])
ylim([0 100])
xlabel('Hz')
ylabel('{\muV}^2/Hz')
title(['PSD of unfiltered & filtered ' ch_names{10}])
...
plot(f(1:end), PSD_unfiltered(1:end,11),'linewidth',1);
hold on
plot(f(1:end), PSD_filtered(1:end,11),'linewidth',1);
xlim([0 80])
ylim([0 100])
xlabel('Hz')
ylabel('{\muV}^2/Hz')
title(['PSD of unfiltered & filtered ' ch_names{11}])
legend('unfiltered','filtered')
...
plot(f(1:end), PSD_unfiltered(1:end,53),'linewidth',1);
hold on
plot(f(1:end), PSD_filtered(1:end,53),'linewidth',1);
xlim([0 80])
ylim([0 100])
xlabel('Hz')
ylabel('{\muV}^2/Hz')
...
%% Stage 01 | Plot the markers of the events over the n filtered signals 
...
% %about 1 minutes. 
% %We can limit x-axis (uncomment following instruction)
%xlim([300 600])
        
%% Stage 01 | Extract epochs [-0.2 0.8]s around each stimulus presentation--> 3-D matrix
...
for i=1:r
    ...
    lim2=stim_samples(i)+epoch_lim(2)*srate-1; %eliminate 1 to have m_ep samples in each epoch, otherwise m_ep+1
    Xepoched(:,:,i)=X(:,lim1:lim2);
end

X=Xepoched; % 3D matrix with size n x m_ep x r

%% Stage 01 | Concatenation of extracted epochs 3-D matrix --> 2-D matrix
...
%% Stage 01 | Identification and plot of bad channels 
...
%the following figure highlights in red the identified bad channels (not required in the text of the lab activity)
t=[0:1:m_conc-1]/srate;
...
%% Stage 01 | Removal of bad channels
...
%% Stage 01 | Saving at the end of these preprocessing stage

save sub-035_PreprocessStep1 X ch_names srate stim_types index_bad index_good
