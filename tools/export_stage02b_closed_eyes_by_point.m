function export_stage02b_closed_eyes_by_point(dpi)
%EXPORT_STAGE02B_CLOSED_EYES_BY_POINT Run Stage 02B section-by-section and save all figures.
...
script_dir = fullfile(repo_root, '02_artifact_removal_ica', '02_ica_cleaning_closed_eyes');
script_path = fullfile(script_dir, 'ica_closed_eyes_pipeline.m');
...
% Add EEGLAB if available (needed by plot_IC/topoplot in point 7)
eeglab_candidates = {
...
point_num = [];
for i = 1:numel(lines)
    tok = regexp(lines{i}, '^%%\s*Closed-Eyes ICA Stage Point\s+(\d+)', 'tokens', 'once');
    if ~isempty(tok)
...
if isempty(point_idx)
    error('No "Closed-Eyes ICA Stage Point" sections found in %s', script_path);
end
...
fprintf('Running Stage 02B by point from: %s\n', script_path);
...
for k = 1:numel(point_idx)
    ...
    marker = sprintf('pre_ex4_%d_%d', p, k);
    ...
    prefix = sprintf('stage02b_p%02d', p);
