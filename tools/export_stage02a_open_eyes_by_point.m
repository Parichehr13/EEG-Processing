function export_stage02a_open_eyes_by_point(dpi)
%EXPORT_STAGE02A_OPEN_EYES_BY_POINT Run Stage 02A section-by-section and save all figures.
...
script_dir = fullfile(repo_root, '02_artifact_removal_ica', '01_ica_cleaning_open_eyes');
script_path = fullfile(script_dir, 'ica_open_eyes_pipeline.m');
...
point_num = [];
for i = 1:numel(lines)
    tok = regexp(lines{i}, '^%%\s*Open-Eyes ICA Stage Point\s+(\d+)', 'tokens', 'once');
    if ~isempty(tok)
...
if isempty(point_idx)
    error('No "Open-Eyes ICA Stage Point" sections found in %s', script_path);
end
...
fprintf('Running Stage 02A by point from: %s\n', script_path);
...
for k = 1:numel(point_idx)
    ...
    marker = sprintf('pre_ex3_%d_%d', p, k);
    ...
    prefix = sprintf('stage02a_p%02d', p);
