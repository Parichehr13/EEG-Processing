function export_exercise9_by_point(dpi)
%EXPORT_EXERCISE9_BY_POINT Run Exercise 9a/9b section-by-section and save figures.
...
ex_dir = fullfile(repo_root, 'exercise9');
...
run_script_by_point(ex_dir, fig_dir, 'Exercise9a_Solution.m', 'exercise9a_sub003', dpi, 'sub-003_PreprocessStep2.mat');
run_script_by_point(ex_dir, fig_dir, 'Exercise9a_Solution.m', 'exercise9a_sub035', dpi, 'sub-035_PreprocessStep2.mat');
run_script_by_point(ex_dir, fig_dir, 'Exercise9b_Solution.m', 'exercise9b', dpi, '');

all_png = {
    'exercise9a_sub003_p07_fig_001.png', ...
    'exercise9a_sub003_p08_fig_001.png', ...
    'exercise9a_sub035_p07_fig_001.png', ...
    'exercise9a_sub035_p08_fig_001.png', ...
    'exercise9b_p02_fig_001.png', ...
    'exercise9b_p03_fig_001.png'
};
manifest_path = fullfile(fig_dir, 'exercise9_manifest.json');
write_manifest(manifest_path, all_png);

fprintf('Exercise 9 export complete.\n');
end

function run_script_by_point(ex_dir, fig_dir, script_name, prefix_root, dpi, load_file)
script_path = fullfile(ex_dir, script_name);
raw = fileread(script_path);
if nargin >= 6 && ~isempty(load_file)
    raw = regexprep(raw, 'load\s+sub-\d+_PreprocessStep2\.mat[^\r\n]*', ['load ' load_file], 'once');
end
...
point_num = [];
for i = 1:numel(lines)
    tok = regexp(lines{i}, '^\s*%%\s*Exercise\s+9[ab]\s+Point(?:s)?\s+(\d+)', 'tokens', 'once');
    if ~isempty(tok)
...
if isempty(point_idx)
    error('No Exercise 9 point sections found in %s', script_path);
end
