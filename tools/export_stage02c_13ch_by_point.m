function export_stage02c_13ch_by_point(dpi)
%EXPORT_STAGE02C_13CH_BY_POINT Run Stage 02C section-by-section and save all figures.
...
script_dir = fullfile(repo_root, '02_artifact_removal_ica', '03_ica_cleaning_13ch_comparison');
script_path = fullfile(script_dir, 'ica_13ch_pipeline.m');
...
point_num = [];
for i = 1:numel(lines)
    tok = regexp(lines{i}, '^%%\s*13-Channel ICA Stage Point\s+(\d+)', 'tokens', 'once');
    if ~isempty(tok)
...
if isempty(point_idx)
    error('No "13-Channel ICA Stage Point" sections found in %s', script_path);
end
...
fprintf('Running Stage 02C by point from: %s\n', script_path);
...
for k = 1:numel(point_idx)
    ...
    marker = sprintf('pre_ex5_%d_%d', p, k);
    ...
    prefix = sprintf('stage02c_p%02d', p);
