function export_stage02d_rest_task_rest_by_point(dpi)
%EXPORT_STAGE02D_REST_TASK_REST_BY_POINT Run Stage 02D section-by-section and save all figures.
...
script_dir = fullfile(repo_root, '02_artifact_removal_ica', '04_rest_task_rest_analysis');
script_path = fullfile(script_dir, 'rest_task_rest_ica_pipeline.m');
...
% Add EEGLAB if available (needed by plot_IC/topoplot)
eeglab_candidates = {
...
point_num = [];
for i = 1:numel(lines)
    tok = regexp(lines{i}, '^%%\s*REST-TASK-REST ICA Stage Point\s+(\d+)', 'tokens', 'once');
    if ~isempty(tok)
...
if isempty(point_idx)
    error('No "REST-TASK-REST ICA Stage Point" sections found in %s', script_path);
end
...
fprintf('Running Stage 02D by point from: %s\n', script_path);
...
for k = 1:numel(point_idx)
    ...
    marker = sprintf('pre_ex6_%d_%d', p, k);
    ...
    prefix = sprintf('stage02d_p%02d', p);
